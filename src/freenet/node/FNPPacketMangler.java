...
public class FNPPacketMangler implements OutgoingPacketMangler {
	...
	*/
	 ...
	 */
	private void processDecryptedAuthAnon(final byte[] payload, final PluginAddress replyTo) {
	...
	private void processDecryptedAuthAnonReply(final byte[] payload, final PluginAddress replyTo, final PeerPacketTransport peerTransport) {
	 ...
	 */
	private void processDecryptedAuth(final byte[] payload, final PeerPacketTransport peerTransport, final PluginAddress replyTo, final boolean oldOpennetPeer) {
		PeerNode pn = peerTransport.pn;
	...
	private void processJFKMessage1(byte[] payload, int offset, PeerPacketTransport peerTransport, PluginAddress replyTo, boolean unknownInitiator, int setupType, int negType) {
		...
		if(payload.length < nonceSize + modulusLength + 3 + (unknownInitiator ? NodeCrypto.IDENTITY_LENGTH : 0)) {
			Logger.error(this, "Packet too short from "+peerTransport+": "+payload.length+" after decryption in JFK(1), should be "+(nonceSize + modulusLength));
			return;
		}
		// get Ni'
	...
	private void processJFKMessage2(byte[] payload, int inputOffset, PeerPacketTransport peerTransport, PluginAddress replyTo, boolean unknownInitiator, int setupType, int negType)
	{
		...
		sendJFKMessage3(1, negType, 3, myNi, nonceResponder, hisExponential, authenticator, peerTransport, replyTo, unknownInitiator, setupType);
	...
	private void processJFKMessage3(byte[] payload, int inputOffset, PeerPacketTransport peerTransport, PluginAddress replyTo, boolean oldOpennetPeer, boolean unknownInitiator, int setupType, int negType)
	{
		...
		if(message4 != null) {
			...
			if(logDEBUG) Logger.debug(this, "No message4 found for "+HexUtil.bytesToHex(authenticator)+" responderExponential "+Fields.hashCode(responderExponential)+" initiatorExponential "+Fields.hashCode(initiatorExponential)+" nonceResponder "+Fields.hashCode(nonceResponder)+" nonceInitiator "+Fields.hashCode(nonceInitiatorHashed)+" address "+HexUtil.bytesToHex(replyTo.getBytes()));
		}
		...
		if(negType < 9) {
		    ...
		    if(!ECDSA.verify(Curves.P256, pn.peerECDSAPubKey(), sig, toVerify)) {
	              Logger.error(this, "The ECDSA signature verification has failed!! JFK(3) - "+peerTransport.getAddress());
	                return;
		    }
		}
	...
	private boolean processJFKMessage4(byte[] payload, int inputOffset, PeerPacketTransport peerTransport, PluginAddress replyTo, boolean oldOpennetPeer, boolean unknownInitiator, int setupType, int negType)
	{
	    ...
	    if(negType < 9) { // DSA sig     
	        ...
	        if(!ECDSA.verify(Curves.P256, peerTransport.pn.peerECDSAPubKey(), sig, locallyGeneratedText)) {
	            Logger.error(this, "The ECDSA signature verification has failed!! JFK(4) - "+peerTransport.getAddress()+" length "+locallyGeneratedText.length+" hisRef "+hisRef.length+" hash "+Fields.hashCode(hisRef)+" myRef "+peerTransport.jfkMyRef.length+" hash "+Fields.hashCode(peerTransport.jfkMyRef)+" boot ID "+bootID);
	            return true;
	        }
	    }
	...
	private void sendJFKMessage3(int version,final int negType,int phase,byte[] nonceInitiator,byte[] nonceResponder,byte[] hisExponential, byte[] authenticator, final PeerPacketTransport peerTransport, final PluginAddress replyTo, final boolean unknownInitiator, final int setupType)
	{
		...
		if(logDEBUG) Logger.debug(this, "The shared Master secret is : "+HexUtil.bytesToHex(computedExponential)+ " for " + peerTransport);
		/* 0 is the outgoing key for the initiator, 7 for the responder */
		peerTransport.outgoingKey = computeJFKSharedKey(computedExponential, nonceInitiatorHashed, nonceResponder, "0");
		peerTransport.incommingKey = computeJFKSharedKey(computedExponential, nonceInitiatorHashed, nonceResponder, "7");
		peerTransport.jfkKe = computeJFKSharedKey(computedExponential, nonceInitiatorHashed, nonceResponder, "1");
		peerTransport.jfkKa = computeJFKSharedKey(computedExponential, nonceInitiatorHashed, nonceResponder, "2");

		peerTransport.hmacKey = computeJFKSharedKey(computedExponential, nonceInitiatorHashed, nonceResponder, "3");
		peerTransport.ivKey = computeJFKSharedKey(computedExponential, nonceInitiatorHashed, nonceResponder, "4");
		peerTransport.ivNonce = computeJFKSharedKey(computedExponential, nonceInitiatorHashed, nonceResponder, "5");
	 ...
	 */
	private void sendJFKMessage4(int version,int negType,int phase,byte[] nonceInitiatorHashed,byte[] nonceResponder,byte[] initiatorExponential,byte[] responderExponential, BlockCipher c, byte[] Ke, byte[] Ka, byte[] authenticator, byte[] hisRef, PeerPacketTransport peerTransport, PluginAddress replyTo, boolean unknownInitiator, int setupType, long newTrackerID, boolean sameAsOldTrackerID) {
		PeerNode pn = peerTransport.pn;
		if(logMINOR)
